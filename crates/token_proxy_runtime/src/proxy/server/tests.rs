...
#[derive(Clone)]
struct MockRawChunkedSequenceResponse {
    status: StatusCode,
    chunks: Vec<Bytes>,
    content_type: String,
}
...
#[derive(Clone)]
struct MockRawChunkedSequenceUpstreamState {
    responses: Arc<Mutex<Vec<MockRawChunkedSequenceResponse>>>,
    requests: Arc<Mutex<Vec<RecordedRequest>>>,
}
...
async fn mock_raw_chunked_sequence_upstream_handler(
    State(state): State<Arc<MockRawChunkedSequenceUpstreamState>>,
    headers: HeaderMap,
    uri: Uri,
    body: Body,
) -> axum::response::Response {
    ...
    let response = {
        ...
        if responses.len() > 1 {
            ...
            responses
                .first()
                .cloned()
                .expect("chunked sequence mock must keep fallback response")
        }
    };
    let stream =
        futures_util::stream::unfold((0usize, response.chunks), |(index, chunks)| async move {
    ...
    axum::response::Response::builder()
        ...
        .expect("build raw chunked sequence mock response")
}

async fn spawn_mock_raw_chunked_sequence_upstream(
    responses: Vec<(StatusCode, Vec<Bytes>, &'static str)>,
) -> MockUpstream {
    let requests = Arc::new(Mutex::new(Vec::new()));
    let responses = responses
        .into_iter()
        .map(
            |(status, chunks, content_type)| MockRawChunkedSequenceResponse {
                status,
                chunks,
                content_type: content_type.to_string(),
            },
        ...
        .collect::<Vec<_>>();
    let state = Arc::new(MockRawChunkedSequenceUpstreamState {
        responses: Arc::new(Mutex::new(responses)),
        requests: requests.clone(),
    });
    let app = Router::new()
        .route("/{*path}", any(mock_raw_chunked_sequence_upstream_handler))
        .with_state(state);
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0")
        .await
        .expect("bind raw chunked sequence mock upstream");
    let addr: SocketAddr = listener.local_addr().expect("mock local addr");
    let task = tokio::spawn(async move {
        axum::serve(listener, app)
            .await
            .expect("raw chunked sequence mock upstream server should run");
    });
    MockUpstream {
        base_url: format!("http://{addr}"),
        requests,
...
async fn build_test_state_handle_with_paths(
    ...
    let codex_accounts = Arc::new(
        token_proxy_account_codex::CodexAccountStore::new(&paths, app_proxy.clone())
            .expect("codex store"),
    );
            ...
            for upstream in &group.items {
                ...
                if let Some(account_id) = upstream.xai_account_id.as_deref() {
                    xai_accounts
                        .save_record_for_test(
                            account_id.to_string(),
                            token_proxy_account_xai::XaiTokenRecord {
...
async fn seed_kiro_account(
    ...
    state_guard
        ...
        .save_record_for_test(
            storage_account_id.to_string(),
            token_proxy_account_kiro::KiroTokenRecord {
...
async fn assert_responses_stream_retries_same_upstream_once_after_split_prelude_capacity_error() {
    let primary = spawn_mock_raw_chunked_sequence_upstream(vec![
        (
            StatusCode::OK,
            vec![
                Bytes::from(
                    "event: response.created\n\
data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_capacity\",\"model\":\"gpt-5\"}}\n\n",
                ),
                Bytes::from(
                    "event: response.failed\n\
data: {\"type\":\"response.failed\",\"response\":{\"id\":\"resp_capacity\",\"object\":\"response\",\"status\":\"failed\",\"error\":{\"code\":\"server_is_overloaded\",\"message\":\"capacity test uses codex server overloaded code\"}}}\n\n",
                ),
            ],
            "text/event-stream",
        ),
...
data: {\"type\":\"response.output_text.delta\",\"delta\":\"same upstream stream recovered\"}\n\n\
event: response.completed\n\
data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_recovered\",\"object\":\"response\",\"created_at\":123,\"model\":\"gpt-5\",\"status\":\"completed\",\"output\":[{\"type\":\"message\",\"id\":\"msg_1\",\"status\":\"completed\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"same upstream stream recovered\"}]}],\"usage\":{\"input_tokens\":1,\"output_tokens\":2,\"total_tokens\":3}}}\n\n\
data: [DONE]\n\n",
            )],
            "text/event-stream",
        ),
    ...
    let data_dir = next_test_data_dir("responses_capacity_split_prelude_same_upstream_retry");
    let state = build_test_state_handle(config, data_dir.clone()).await;
    ...
    assert_eq!(response_status, StatusCode::OK);
    assert!(response_text.contains("same upstream stream recovered"));
    assert!(
        !response_text.contains("Selected model is at capacity"),
        "capacity prelude error must not leak to client after retry: {response_text}"
    );
    assert_eq!(
        primary_requests.len(),
        2,
        "split prelude capacity response should retry the same upstream once"
    );
    for request in primary_requests {
        assert_eq!(request.path, CODEX_RESPONSES_PATH);
        assert_eq!(
            request.body["model"].as_str(),
            Some("gpt-5"),
            "capacity retry must keep the caller-requested model"
...
fn models_index_uses_xai_builtin_catalog_without_models_endpoint() {
    run_async(async {
        ...
        assert_eq!(status, StatusCode::OK);
        assert_eq!(ids, token_proxy_account_xai::BUILTIN_MODELS);
...
#[test]
fn responses_stream_request_retries_same_upstream_once_after_split_prelude_capacity_error() {
    run_async(async {
        assert_responses_stream_retries_same_upstream_once_after_split_prelude_capacity_error()
            .await;
    });
}
...
#[test]
fn openai_image_generation_codex_bridge_retries_incomplete_to_next_upstream() {
    run_async(async {
        let primary = spawn_mock_raw_upstream(
            StatusCode::OK,
            Bytes::from(
                "data: {\"type\":\"response.incomplete\",\"response\":{\"id\":\"resp_img_incomplete\",\"object\":\"response\",\"created_at\":1710000020,\"model\":\"gpt-5.4-mini\",\"status\":\"incomplete\",\"incomplete_details\":{\"reason\":\"max_output_tokens\"},\"output\":[]}}\n\n\
data: [DONE]\n\n",
            ),
            "text/event-stream",
        )
        ...
        let fallback = spawn_mock_raw_upstream(
            StatusCode::OK,
            Bytes::from(
                "data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_img_after_incomplete\",\"object\":\"response\",\"created_at\":1710000021,\"model\":\"gpt-5.4-mini\",\"status\":\"completed\",\"output\":[{\"type\":\"image_generation_call\",\"id\":\"ig_1\",\"result\":\"ZmFpbG92ZXI=\",\"output_format\":\"png\"}]}}\n\n\
data: [DONE]\n\n",
            ),
            "text/event-stream",
        )
        ...
        let data_dir = next_test_data_dir("openai_images_generation_incomplete_retry");
        let state = build_test_state_handle(config, data_dir.clone()).await;
...
#[test]
fn openai_image_generation_codex_bridge_retries_empty_output_to_next_upstream() {
    run_async(async {
        let primary = spawn_mock_raw_upstream(
            StatusCode::OK,
            Bytes::from(
                "data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_img_empty_retry\",\"object\":\"response\",\"created_at\":1710000024,\"model\":\"gpt-5.4-mini\",\"status\":\"completed\",\"output\":[]}}\n\n\
data: [DONE]\n\n",
            ),
            "text/event-stream",
        )
        ...
        let fallback = spawn_mock_raw_upstream(
            StatusCode::OK,
            Bytes::from(
                "data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_img_after_empty\",\"object\":\"response\",\"created_at\":1710000025,\"model\":\"gpt-5.4-mini\",\"status\":\"completed\",\"output\":[{\"type\":\"image_generation_call\",\"id\":\"ig_1\",\"result\":\"ZW1wdHktZmFpbG92ZXI=\",\"output_format\":\"png\"}]}}\n\n\
data: [DONE]\n\n",
            ),
            "text/event-stream",
        )
        ...
        let data_dir = next_test_data_dir("openai_images_generation_empty_output_retry");
        let state = build_test_state_handle(config, data_dir.clone()).await;
        ...
        let response = proxy_request(
            ...
            Body::from(
                json!({
                    "model": "gpt-image-2",
                    "prompt": "draw after empty output"
                })
                .to_string(),
            ),
        )
...
#[test]
fn openai_image_generation_codex_bridge_content_filter_incomplete_does_not_retry() {
    run_async(async {
        let primary = spawn_mock_raw_upstream(
            StatusCode::OK,
            Bytes::from(
                "data: {\"type\":\"response.incomplete\",\"response\":{\"id\":\"resp_img_filtered\",\"object\":\"response\",\"created_at\":1710000022,\"model\":\"gpt-5.4-mini\",\"status\":\"incomplete\",\"incomplete_details\":{\"reason\":\"content_filter\"},\"output\":[]}}\n\n\
data: [DONE]\n\n",
            ),
            "text/event-stream",
        )
        ...
        let fallback = spawn_mock_raw_upstream(
            StatusCode::OK,
            Bytes::from(
                "data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_img_unused\",\"object\":\"response\",\"created_at\":1710000023,\"model\":\"gpt-5.4-mini\",\"status\":\"completed\",\"output\":[{\"type\":\"image_generation_call\",\"id\":\"ig_1\",\"result\":\"dW51c2Vk\",\"output_format\":\"png\"}]}}\n\n\
data: [DONE]\n\n",
            ),
            "text/event-stream",
        )
        ...
        let data_dir = next_test_data_dir("openai_images_generation_content_filter_no_retry");
        let state = build_test_state_handle(config, data_dir.clone()).await;
        ...
        assert_eq!(response_status, StatusCode::BAD_REQUEST);
        assert!(response_text.contains("content_filter"));
        assert_eq!(primary_requests.len(), 1);
        assert!(fallback_requests.is_empty());
    });
}
