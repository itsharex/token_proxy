...
fn stream_chat_to_responses_handles_chunk_boundaries_and_emits_created_delta_done_and_logs_usage() {
    run_async(async {
        ...
        let response_id = created["response_id"].as_str().expect("response_id");
        assert!(response_id.starts_with("resp_proxy_"));

        let delta_1 = parse_sse_json(&chunks[1]).expect("json");
        assert_eq!(delta_1["type"], json!("response.output_text.delta"));
        assert_eq!(delta_1["delta"], json!("Hi"));
        assert_eq!(delta_1["sequence_number"], json!(1));

        let delta_2 = parse_sse_json(&chunks[2]).expect("json");
        assert_eq!(delta_2["type"], json!("response.output_text.delta"));
        assert_eq!(delta_2["delta"], json!("!"));
        assert_eq!(delta_2["sequence_number"], json!(2));

        let completed = parse_sse_json(&chunks[3]).expect("json");
        assert_eq!(completed["type"], json!("response.completed"));
        assert_eq!(completed["response_id"], json!(response_id));
        assert_eq!(completed["status"], json!("success"));
